    ...
    handler: async (_request: Request, ctx?: ExtensionContext) => {
      ...
      try {
        capabilityBlocked = await awaitCommerceOperation(
          requireCapability(auth.supabase, auth.companyId, CAPABILITY.shopify_sync),
          workStartDeadlineMs,
          'authorize Shopify sync capability',
        )
      } catch (error) {
        if (error instanceof CommerceSyncTimeoutError) return deadlineResponse()
        throw error
      ...
      try {
        rl = await awaitCommerceOperation(
          ...
          workStartDeadlineMs,
          'rate limit manual Shopify sync',
        )
      } catch (error) {
        if (error instanceof CommerceSyncTimeoutError) return deadlineResponse()
        throw error
      ...
      try {
        connectionResult = await awaitCommerceOperation(
          connectionQuery,
          workStartDeadlineMs,
          'fetch manual Shopify connection',
        )
      } catch (error) {
        if (error instanceof CommerceSyncTimeoutError) return deadlineResponse()
        throw error
      ...
      let claim: ClaimedCommerceConnection<ShopifyConnection> | null
      try {
        ...
        log.error('[shopify] Manual sync claim failed', {
      ...
      try {
        if (Date.now() >= workStartDeadlineMs) {
          ...
          // Bounded like the cron: without a deadline a huge first sync would
          // be killed at the dispatcher's maxDuration with no cursor persisted;
        ...
        try {
          ...
          cleanupError = error
          log.error('[shopify] Manual sync claim cleanup failed', {
            message: error instanceof Error ? error.message : String(error),
            connection_id: connection.id,
          })
        }
