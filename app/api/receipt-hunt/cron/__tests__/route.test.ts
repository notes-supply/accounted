...
describe('GET /api/receipt-hunt/cron', () => {
  ...
  it('runs each allowlisted company and aggregates what was proposed', async () => {
    ...
    expect(mockHuntCompany.mock.calls[0][3]).toEqual(expect.objectContaining({
      searchMail: false,
      signal: expect.any(AbortSignal),
      deadlineAt: expect.any(String),
    }))
    expect(Date.parse(mockHuntCompany.mock.calls[0][3].deadlineAt)).toBeGreaterThan(Date.now())
  })

  it('skips a non-entitled company before Gmail, model, storage, or proposal processing', async () => {
    process.env.RECEIPT_HUNT_COMPANY_IDS = 'co-free'
    ...
    expect(body.results).toEqual([
      expect.objectContaining({ companyId: 'co-free', skippedNoAiEntitlement: true }),
    ])
    expect(body).toMatchObject({ success: true, total: 1, succeeded: 1, failed: 0 })
  })
  ...
  it('times out one stalled company and continues while the global budget remains', async () => {
    vi.useFakeTimers()
    process.env.RECEIPT_HUNT_COMPANY_IDS = 'co-stalled,co-next'
    mockHuntCompany
      .mockImplementationOnce((_supabase, _companyId, _runId, options) =>
        new Promise((_resolve, reject) => {
          const signal = (options as { signal: AbortSignal }).signal
          signal.addEventListener('abort', () => reject(signal.reason), { once: true })
        }))
      .mockResolvedValueOnce({ companyId: 'co-next', candidates: 1, poolSize: 0, proposed: 0 })
    ...
    expect(body.failures[0].error).toBe('RECEIPT_HUNT_DEADLINE_EXCEEDED')
    expect((mockHuntCompany.mock.calls[0][3] as { signal: AbortSignal }).signal.aborted).toBe(true)
  })

  it('does not continue or report until a delayed staging RPC settles after timeout', async () => {
    vi.useFakeTimers()
    process.env.RECEIPT_HUNT_COMPANY_IDS = 'co-stalled,co-next'
    ...
    expect(mockHuntCompany).toHaveBeenCalledTimes(1)
    expect(settleStalled).toEqual(expect.any(Function))
