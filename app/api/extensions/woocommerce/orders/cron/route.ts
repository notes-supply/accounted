...
interface WooCommerceCronResult extends CommerceSyncTotals {
  connectionId: string
...
function toCronResult(
  ...
  status: WooCommerceCronResult['status'],
): WooCommerceCronResult {
  return {
    connectionId,
...
export const GET = withCronContext('cron.woocommerce_order_sync', async (_request, ctx) => {
  ...
  const selectionTimeoutResponse = (completedResults: WooCommerceCronResult[] = []) => {
    ctx.log.error(
      'woocommerce sync selection exceeded time budget',
  ...
  let selection
  try {
    selection = await claimCommerceOrderSyncConnections<WooCommerceConnection>({
      supabase,
      table: 'woocommerce_connections',
      capability: CAPABILITY.woocommerce_sync,
      selectionDeadlineMs: deadlineMs,
      cleanupDeadlineMs,
      log: ctx.log,
    })
  } catch (error) {
    ctx.log.error('failed to select and claim woocommerce connections', error as Error)
  ...
  const results: WooCommerceCronResult[] = []
  ...
  for (let index = 0; index < selection.claims.length; index++) {
    ...
    if (Date.now() >= deadlineMs) {
      ...
      for (const unstarted of selection.claims.slice(index)) {
        try {
          ...
          ctx.log.error('failed to restore unstarted woocommerce sync claim', error as Error, {
    ...
    try {
      const syncSummary = await syncWooCommerceOrders(supabase, connection, ctx.log, deadlineMs)
      summary = syncSummary
      ...
      if (!syncSummary.revoked) {
        try {
          ...
          ctx.log.error('failed to release woocommerce sync claim', releaseError as Error, {
    ...
    if (queuePersistenceError) {
      for (const unstarted of selection.claims.slice(index + 1)) {
        try {
          ...
          ctx.log.error('failed to restore unstarted woocommerce sync claim', restoreError as Error, {
            connectionId: unstarted.connection.id,
          })
        }
      }
