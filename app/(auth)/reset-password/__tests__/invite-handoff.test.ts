...
describe('a session that still owes an MFA step-up', () => {
  ...
  it('defers a BankID-linked user under the server policy', async () => {
    const jar = installCookieJar({ [INVITE_COOKIE_NAME]: TOKEN })
    respondWith(200)
  ...
  it('defers on self-hosted when the server requires MFA', async () => {
  ...
  it('does not inspect the session when the public policy disables MFA', async () => {
