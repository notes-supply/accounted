...
describe('updateSession redirect destinations', () => {
  const envBackup = {
    ...
    serverRequire: process.env.REQUIRE_MFA,
  ...
  afterEach(() => {
    ...
    if (envBackup.serverRequire === undefined) delete process.env.REQUIRE_MFA
    else process.env.REQUIRE_MFA = envBackup.serverRequire
  ...
  describe('MFA server policy', () => {
    it('does not redirect when REQUIRE_MFA is explicitly false', async () => {
      state.user = SIGNED_IN
      state.aal = { currentLevel: 'aal1', nextLevel: 'aal2' }
    ...
    it('enforces MFA on self-hosted when the server requires it', async () => {
      process.env.REQUIRE_MFA = 'true'
      process.env.NEXT_PUBLIC_REQUIRE_MFA = 'true'
    ...
    it('does not exempt BankID-linked accounts from the server policy', async () => {
      process.env.REQUIRE_MFA = 'true'
      process.env.NEXT_PUBLIC_REQUIRE_MFA = 'true'
    ...
    it('fails closed when assurance evidence is absent', async () => {
      process.env.REQUIRE_MFA = 'true'
      state.user = SIGNED_IN
      state.aal = null

      const pageResponse = await run('/settings/tax')
      const apiResponse = await run('/api/invoices')

      expect(pageResponse.status).toBe(307)
      expect(new URL(locationOf(pageResponse)!).pathname).toBe('/mfa/verify')
