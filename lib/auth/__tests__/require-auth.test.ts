...
describe('requireAuth', () => {
  ...
  it('does not exempt bankid_linked users from the server policy', async () => {
    vi.stubEnv('REQUIRE_MFA', 'true')
    vi.stubEnv('NEXT_PUBLIC_SELF_HOSTED', 'true')
  ...
  it.each([
    ...
    { data: { currentLevel: 'aal2', nextLevel: 'aal2' }, error: { message: 'lookup failed' } },
  ])('fails closed when assurance evidence is missing or invalid: %j', async assurance => {
    vi.stubEnv('REQUIRE_MFA', 'true')
    const getClaims = vi.fn().mockResolvedValue({ data: { claims: CLAIMS }, error: null })
    const getAuthenticatorAssuranceLevel = vi.fn().mockResolvedValue(assurance)
    useSupabase({ getClaims, mfa: { getAuthenticatorAssuranceLevel } })
