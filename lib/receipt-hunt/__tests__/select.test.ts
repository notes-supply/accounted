...
describe('selectProposals', () => {
  ...
  it('uses the configured confidence floor during selection', () => {
    const original = process.env.RECEIPT_HUNT_MIN_CONFIDENCE
    process.env.RECEIPT_HUNT_MIN_CONFIDENCE = '0.6'
    try {
      const sharedFloorMatch = item({}, {
        supplier: { name: 'Circle K' },
        invoice: { invoiceDate: '2026-05-02', currency: 'SEK' },
        totals: { total: 500, vatAmount: 0 },
      })
      expect(selectProposals([tx()], [sharedFloorMatch], noSuppression)).toHaveLength(1)
    } finally {
